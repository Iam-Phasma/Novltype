...
(function initBgGrid() {
  ...
  const FADE_SPEED = 0.08; // per-frame fade step (doubled)
  ...
  function draw() {
    ...
    // Bright grid lines radiating from cursor
    if (mouseX > -9000) {
      const LINE_RADIUS = 4.0; // cells
      const LINE_PEAK = isLight ? 0.55 : 0.5;
      const LINE_BASE = 0.045;
      const lr = isLight ? 107 : 107;
      const lg = isLight ? 107 : 107;
      const lb = isLight ? 255 : 255;

      const cMin = Math.max(
        0,
        Math.floor((mouseX - LINE_RADIUS * CELL) / CELL),
      );
      const cMax = Math.min(
        cols,
        Math.ceil((mouseX + LINE_RADIUS * CELL) / CELL),
      );
      const rMin = Math.max(
        0,
        Math.floor((mouseY - LINE_RADIUS * CELL) / CELL),
      );
      const rMax = Math.min(
        rows,
        Math.ceil((mouseY + LINE_RADIUS * CELL) / CELL),
      );

      ctx.lineWidth = 1;
      for (let r = rMin; r <= rMax; r++) {
        for (let c = cMin; c <= cMax; c++) {
          // Horizontal segment (c,r)→(c+1,r)
          if (c < cols && (cellVisible(c, r - 1) || cellVisible(c, r))) {
            ...
            if (dist < LINE_RADIUS) {
              const a =
                LINE_BASE + (LINE_PEAK - LINE_BASE) * (1 - dist / LINE_RADIUS);
